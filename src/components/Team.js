...
function SocialIcons() {
  return (
    <div className="flex items-center gap-3">
      <a href="#contact" aria-label="Email" className="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-slate-100 text-slate-600 transition-colors hover:text-slate-900">
      ...
      <a href="#contact" aria-label="LinkedIn" className="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-slate-100 text-slate-600 transition-colors hover:text-slate-900">
      ...
      <a href="#publications" aria-label="Scholar" className="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-slate-100 text-slate-600 transition-colors hover:text-slate-900">
    ...
    <section id="people" className="py-14 sm:py-16 lg:py-20">
      <div className="mx-auto max-w-[1280px]">
        <div className="text-center">
          ...
          <p className={`${inter.className} mx-auto mt-4 max-w-4xl text-base text-[#355178] sm:text-[1.05rem]`}>
        ...
        <Link href="/faculty" className="group mt-10 block">
          <article className="rounded-2xl border border-slate-200 bg-white px-6 py-6 shadow-[0_2px_14px_rgba(15,23,42,0.06)] transition-all duration-200 hover:border-slate-300 hover:shadow-[0_8px_28px_rgba(15,23,42,0.1)] sm:px-8 sm:py-8">
            <div className="flex flex-col gap-5 sm:flex-row sm:items-start">
              ...
              <div className="min-w-0 flex-1">
                <span className={`${inter.className} inline-flex rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-700`}>
                  Principal Investigator
                </span>
                <div className="mt-2 flex flex-wrap items-center gap-2">
                  <h3 className={`${playfair.className} text-[2rem] font-semibold leading-tight text-[#0D1733] transition-colors group-hover:text-[#1B2B53] sm:text-[2.3rem]`}>
                    Dr. K. V. Vamsi
                  </h3>
                  <span className="inline-flex items-center gap-1 rounded-full border border-blue-200 bg-blue-50 px-2.5 py-0.5 text-[11px] font-medium text-blue-700 opacity-0 transition-opacity group-hover:opacity-100">
                    View Profile
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="h-3 w-3"><path d="M5 12h14M13 6l6 6-6 6"/></svg>
                  </span>
                </div>
                <p className={`${inter.className} mt-2 text-[1.05rem] leading-relaxed text-[#355178]`}>
                  Assistant Professor, Department of Metallurgical Engineering and Materials Science, IIT Indore
                </p>
                <p className={`${inter.className} mt-4 max-w-4xl text-[1rem] leading-relaxed text-[#4B607E]`}>
        ...
        <div className="mt-10 grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
          {members.map((member, index) => (
            <article key={member.name} className="rounded-2xl border border-slate-200 bg-white px-5 py-6 text-center shadow-[0_2px_14px_rgba(15,23,42,0.06)]">
              <Image src={member.image} alt={member.name} width={96} height={96}
                className="mx-auto h-24 w-24 rounded-full object-cover"
                style={{ objectPosition: `${35 + index * 10}% 35%` }} />
              <h4 className={`${playfair.className} mt-5 text-[1.55rem] font-semibold leading-tight text-[#0D1733]`}>
                {member.name}
              </h4>
              <p className={`${inter.className} mt-2 text-sm font-medium text-[#355178]`}>{member.role}</p>
              <p className={`${inter.className} mt-3 min-h-[56px] text-[0.98rem] leading-relaxed text-[#4B607E]`}>{member.summary}</p>
